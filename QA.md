...
This starts a temporary local server, launches headless Chrome, decodes synthetic PC and Android saves, exercises rendered category/stage/search/type navigation, changed-only filtering, representative edit flows, and a deep Celestials scoped edit, checks rendered iPhone-sized layouts, and writes screenshots plus `latest.json` to `artifacts/mobile-viewport/`.
